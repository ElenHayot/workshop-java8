...
public class Function_01_Test {
    ...
    private Function<Integer, Person> intToPerson = p -> {
    	Person person = new Person();
    	person.setFirstname("first_"+p);
    	person.setLastname("last_"+p);
    	person.setAge(p);
    	person.setPassword("pass_"+p);
    	return person;
    };
    // end::intToPerson[]
    ...
    private Function<Integer, Account> intToAccountWithCompose = p -> personToAccount.compose(intToPerson).apply(p);
    ...
    private Function<Integer, Account> intToAccountWithAndThen = p -> intToPerson.andThen(personToAccount).apply(p);;
