...
public class Stream_01_Test {
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        Predicate<Order> predicateFirstName = p -> p.getCustomer().getFirstname().equals("Johnny");
        Predicate<Pizza> predicatePizzaPrice = p -> p.getPrice() >= 1300;

        // TODO récupérer toutes les commandes dont
        // TODO le prénom du client est "Johnny"
        // TODO dont au moins une pizza a un prix >= 1300
        List<Order> result = orders	.stream().filter(predicateFirstName)
        							.filter(o -> o.getPizzas().stream().anyMatch(predicatePizzaPrice))
    ...
    public void test_stream_findFirst() throws Exception {
        List<Order> orders = new Data().getOrders();
        Predicate<Order> predicateFirstName = o -> o.getCustomer().getFirstname().equals("Sophie");

        // TODO récupérer une commande faite par un client dont le prénom est "Sophie"
        Optional<Order> result = orders.stream().filter(predicateFirstName).findFirst();
    ...
    public void test_stream_max() throws Exception {
        List<Pizza> pizzas = new Data().getPizzas();
        Comparator<Pizza> comparator = Comparator.comparing(Pizza::getPrice);
        
        // TODO Trouver la pizza la plus chère
        Optional<Pizza> result = pizzas.stream().max(comparator);
    ...
    public void test_stream_min() throws Exception {
        ...
        Optional<Pizza> result = pizzas.stream().filter(p -> p.getPrice() >= 950).min(comparator);
